...
DROP POLICY IF EXISTS "Cho phép xem danh sách follow" ON user_follows;
...
DROP POLICY IF EXISTS "Cho phép user tự follow" ON user_follows;
CREATE POLICY "Cho phép user tự follow" ON user_follows
  FOR INSERT WITH CHECK (auth.uid() = follower_id);

DROP POLICY IF EXISTS "Cho phép user tự unfollow" ON user_follows;
