...
function PublicProfilePage() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      if (nextAccessState === 'allowed' && preferences.show_public_posts) {
        const postsResult = await getPublicPostsByUser(userId)
  ...
  const handleFollowToggle = async () => {
    ...
    if (followingStatus) {
      ...
      if (!error) {
        ...
        alert(error?.message?.includes('does not exist') ? 'Tính năng đang bảo trì. Vui lòng chạy file SQL.' : 'Lỗi: ' + (error.message || 'Không thể hủy theo dõi'))
      }
    } else {
      // Follow
      ...
      if (!error) {
        ...
        alert(error?.message?.includes('does not exist') ? 'Tính năng đang bảo trì. Vui lòng chạy file SQL.' : 'Lỗi: ' + (error.message || 'Không thể theo dõi'))
      }
    }
  ...
  const shouldNoIndex = !preferences.allow_search_index

  if (accessState !== 'allowed' && !isSelf) {
    return (
      <>
        <Helmet>
          <title>{displayName} - E-XANH</title>
  ...
  return (
    <>
      <Helmet>
        <title>{displayName} - E-XANH</title>
        <meta name="description" content={profile.bio || 'Thành viên E-XANH yêu thích sống xanh.'} />
        {shouldNoIndex ? <meta name="robots" content="noindex,nofollow" /> : null}
      </Helmet>
      ...
      <div className="shell shell--wide public-profile-page">
        <div className="public-profile-header">
          ...
          <div className="public-profile-info">
            ...
            <p className="public-profile-bio">{profile.bio || 'Thành viên E-XANH yêu thích sống xanh.'}</p>

            <div className="public-profile-meta">
              {profile.area && (
                <div className="public-profile-meta-item">
            ...
            <div className="public-profile-stats">
              <div className="public-profile-stat">
                <span className="public-profile-stat-value">{canShowPosts ? posts.length : 0}</span>
                <span className="public-profile-stat-label">Bài viết</span>
              </div>
              <div className="public-profile-stat">
                <span className="public-profile-stat-value">{followStats.followersCount}</span>
                <span className="public-profile-stat-label">Người theo dõi</span>
              </div>
              <div className="public-profile-stat">
                <span className="public-profile-stat-value">{followStats.followingCount}</span>
        ...
        <div className="public-profile-tabs">
          <button 
            className={`public-profile-tab ${activeTab === 'posts' ? 'is-active' : ''}`}
